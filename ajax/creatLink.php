...
$rew = mysql_query("SELECT `name`,`group` FROM `links777`");
...
//------------ ЗАПОМИНАНИЕ ВРЕМЕНИ СОЗДАНИЯ ССЫЛКИ --------------
	...
	$rew = mysql_query("SELECT `number` FROM `links777` ORDER BY `id` DESC LIMIT 1");
	...
	$prav = mysql_query("INSERT INTO `links777` (`name`,`group`,`links`,`choise`,`background`,`icon`,`screen`,`full_screen`,`title`,`data`,`timeCreat`) VALUES ('".$_POST['name']."','".$_POST['group']."','".$_POST['links']."','".$_POST['choise']."','".$_POST['background']."','".$background[1]."','".$background[2]."','".$background[3]."','".$_POST['title']."','".$now."','".$timeCreat."') ");
	...
	$pes = mysql_query(" SELECT COUNT(*) FROM `links777` WHERE `group` = '".$_POST['group']."' ");
	$row = mysql_fetch_row($pes);
	$count = $row[0];
	
	
	//-- ВСТАВКА НОМЕРА ССЫЛКИ В ГРУППЕ
	mysql_query(" UPDATE `links777` SET `number` = '".$count."' WHERE `id` = '".$id_link."' ");
	
	//-- ВСТАВКА КОЛИЧ. ССЫЛОК В ГРУППЕ
	mysql_query(" UPDATE `group_links` SET `kol` = '".$count."' WHERE `name` = '".$_POST['group']."' ");	

 
//$id_link */


	$pesr = mysql_query(" SELECT * FROM `links777` WHERE `id` = '".$id_link."' ");
