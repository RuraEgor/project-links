...
$mes = array();
$per = mysql_query(" SELECT `name`,`view` FROM `group_links` ORDER BY `number` ASC ");
...
$root = false;
$mes = array();
$per = mysql_query(" SELECT `name` FROM `group_links` ORDER BY `id` ");
...
while ($row = mysql_fetch_assoc($per)){
	...
	if($root != true){
		$myq = "INSERT INTO `group_links` (`name`,`background`,`context`) VALUES ('".$_POST['name']."','".$_POST['background']."','".$_POST['context']."')";
		...
		$pes = mysql_query(" SELECT COUNT(`id`) FROM `group_links` ");
		$row = mysql_fetch_row($pes);
		$count = $row[0];
		
		//-- ВСТАВКА НОМЕРА ССЫЛКИ В ГРУППЕ
		mysql_query(" UPDATE `group_links` SET `number` = '".$count."' WHERE `id` = '".$id_link."' ");
...
$nomElem = mysql_query(" SELECT `name`,`kol`,`number` FROM `group_links` WHERE `id` = '".$nameDelCat."' ");
...
for($i = 0; $i < $paramGr[1]; $i++){
	$provLin = mysql_query(" DELETE FROM `links777` WHERE ( `group` = '".$paramGr[0]."' ) ");
}

//------  УДАЛЕНИЕ САМОЙ КАТЕГОРИИ  ---------
$provLin = mysql_query(" DELETE FROM `group_links` WHERE `id` = '".$nameDelCat."' ");


//-- БЛОК ОПРЕДЕЛЕНИЯ КОЛИЧ. КАТЕГОРИЙ
$pess = mysql_query(" SELECT COUNT(*) FROM `group_links` ");
...
for($i = $paramGr[2]; $i < ($count + 1); $i++){
	...
	mysql_query(" UPDATE `group_links` SET `number`='".$i."' WHERE `number` = '".$x."' ");
}
...
if($UPDATE_GROUP != '&&&&') {
	...
	$per = mysql_query(" SELECT * FROM `group_links` WHERE `id` = $UPDATE_GROUP ");
...
if($UPD_GR_F != '&&&&') {
	...
	$per = mysql_query(" SELECT `name` FROM `group_links` WHERE `id` = '".$UPD_GR_F[0]."' ");
	...
	$per = mysql_query(" SELECT `name` FROM `group_links` WHERE `id` NOT IN ('".$UPD_GR_F[0]."')");
	...
	if($povt_cat) {
		//--  обновление элемента
		$apdate = mysql_query(" UPDATE `group_links` SET `name` = '".$UPD_GR_F[1]."', `background` = '".$UPD_GR_F[2]."',  `context` = '".$UPD_GR_F[3]."',  `view` = '".$UPD_GR_F[4]."'  WHERE id = '".$UPD_GR_F[0]."' ");

	//-----


		if($apdate) { $apdate = mysql_query(" UPDATE `links777` SET `group` = '".$UPD_GR_F[1]."'  WHERE `group` = '".$nameCat."' "); }
	}
